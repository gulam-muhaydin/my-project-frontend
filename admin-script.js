(function () {
    ...
    function renderDashboardWithPending(pending) {
        var users = getUsers();
        ...
        usersBody.innerHTML = userList.map(function (u) {
            var points = getStoredPoints(u.email);
            var statusClass = u.approved ? 'text-success' : (u.paymentSubmitted ? 'text-warning' : 'text-muted');
    ...
    function renderDashboard() {
        getPendingPaymentsAsync(function (pending) {
            renderDashboardWithPending(pending);
        });
    }
